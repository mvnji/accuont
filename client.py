...
class Bot:
	def __init__(self, app_name, auth, device=defaultDevice):
		self.auth = auth
	...
	def sendCodenotpassword(phone_number,pass_you=None):
		tmp = Bot._tmpGeneration()
		enc = rubika.encryption.encryption(tmp)
		while True:
			try:
	...
	def sendSticker(self, group_guid,emoje,access_hash_rec,dc_id,file_id,sticker_id,sticker_set_id):
		return loads(self.enc.decrypt(post(json={"api_version":"5","auth": self.auth,"data_enc":self.enc.encrypt(dumps({
			"method":"sendMessage",
			"input":{
